...
from nonlinear_function_space import DenseNet, Linear, NN, NN_Nik, SingleParam, MySequential, DenseNet_tanh, DenseNet1, DenseNet2, DenseNet3
from nonlinear_utilities import compute_test_error, do_importance_sampling, do_importance_sampling_me
...
#         print('ESS_ave=', ESS_ave, 'ESS_ave percentage=', ESS_ave / self.K_BPF_small)
        ...
        K_BPF_smallRV = 100000
        X_TPFRV1 = self.X_0.repeat(self.K_BPF_smallRV, 1).to(self.device)


        y_observed_KBPFLRV1 = [] #define y_OB for K_BPF_large particles
        for n in range(self.N + 1):
            y_observed_KBPFLRV1.append(self.y_observed[n].repeat(self.K_BPF_smallRV,1).to(self.device))

        g_prodRV1 = 1

        for n in range(self.N+1):

            g_kRV1 = pt.exp(-self.f(X_TPFRV1 - y_observed_KBPFLRV1[n], n * self.delta_t))


            if(n == self.N):
                gphi_kRV1 = g_kRV1 / self.phi_Z_n_(X_TPFRV1,n).squeeze()
              

            elif(n == 0):
                
                gphi_kRV1 = g_kRV1 * self.tildephi_Z_n_(X_TPFRV1, n).squeeze()
          


            else:
                
                gphi_kRV1 = g_kRV1 * self.tildephi_Z_n_(X_TPFRV1,n).squeeze() / self.phi_Z_n_(X_TPFRV1,n).squeeze()
          


          
            g_prodRV1 *= gphi_kRV1.mean().item()
        


            #resample
            weightsRV1 = gphi_kRV1 / pt.sum(gphi_kRV1)
       
            indicesRV1 = pt.multinomial(weightsRV1, self.K_BPF_smallRV, replacement=True)
            ESSRV1 = 1.0 / np.sum((weightsRV1.detach().to('cpu').numpy())**2)
            

            if(ESSRV1 <= self.resample_kappa * self.K_BPF_smallRV):
                X_TPFRV1 = X_TPFRV1[indicesRV1]
                


            
            X_TPFRV1 = (self.delta_t * self.mean_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)).expand(X_TPFRV1.shape[0],-1)\
                            + self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)) * (X_TPFRV1 + self.delta_t * self.b(X_TPFRV1)))\
                    / (self.delta_t + self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)))\
                        + pt.sqrt((self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)) * self.delta_t)\
                                    / (self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)) + self.delta_t))\
                            * pt.randn_like(X_TPFRV1).to(self.device)
        ...
        X_TPFRV2 = self.X_0.repeat(self.K_BPF_smallRV, 1).to(self.device)


        y_observed_KBPFLRV2 = [] #define y_OB for K_BPF_large particles
        for n in range(self.N + 1):
            y_observed_KBPFLRV2.append(self.y_observed[n].repeat(self.K_BPF_smallRV,1).to(self.device))

        g_prodRV2 = 1

        for n in range(self.N+1):

            g_kRV2 = pt.exp(-self.f(X_TPFRV2 - y_observed_KBPFLRV2[n], n * self.delta_t))


            if(n == self.N):
                gphi_kRV2 = g_kRV2 / self.phi_Z_n_(X_TPFRV2,n).squeeze()
              

            elif(n == 0):
                
                gphi_kRV2 = g_kRV2 * self.tildephi_Z_n_(X_TPFRV2, n).squeeze()
          


            else:
                
                gphi_kRV2 = g_kRV2 * self.tildephi_Z_n_(X_TPFRV2,n).squeeze() / self.phi_Z_n_(X_TPFRV2,n).squeeze()
          


          
            g_prodRV2 *= (gphi_kRV2**2).mean().item()
        


            #resample
            weightsRV2 = gphi_kRV2 / pt.sum(gphi_kRV2)
       
            indicesRV2 = pt.multinomial(weightsRV2, self.K_BPF_smallRV, replacement=True)
            ESSRV2 = 1.0 / np.sum((weightsRV2.detach().to('cpu').numpy())**2)
            

            if(ESSRV2 <= self.resample_kappa * self.K_BPF_smallRV):
                X_TPFRV2 = X_TPFRV2[indicesRV2]
                


            
            X_TPFRV2 = (self.delta_t * self.mean_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)).expand(X_TPFRV2.shape[0],-1)\
                            + self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)) * (X_TPFRV2 + self.delta_t * self.b(X_TPFRV2)))\
                    / (self.delta_t + self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)))\
                        + pt.sqrt((self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)) * self.delta_t)\
                                    / (self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)) + self.delta_t))\
                            * pt.randn_like(X_TPFRV2).to(self.device)
            
        EX2 = g_prodRV2
        print("EX2",EX2)
        
        print("RV:", np.sqrt((EX2/(EX1)**2) - 1))
...
class mynonlinear_Solver():
    ...
    def b(self, x):
        ...
        return (x/2 + 25 * x / (1 + pt.sum(x**2, dim=1,keepdim = True)) - x) / self.delta_t
    ...
    def f(self, xx, yy, t):
        exponent = -0.5 * pt.sum((xx**2/ 20 - yy ) ** 2, dim=1)
        return -exponent
    ...
    def train(self):
        ...
        for i in range(ifRV):
            ...
            X_TPFRV1 = self.X_0.repeat(K_BPF_smallRV, 1).to(self.device)
            X_TPFRV2 = self.X_0.repeat(K_BPF_smallRV, 1).to(self.device)


            y_observed_KBPFLRV = []#define y_OB for K_BPF_large particles
            for n in range(self.N + 1):
                y_observed_KBPFLRV.append(self.y_observed[n].repeat(K_BPF_smallRV,1).to(self.device))

            g_prodRV1 = 1

            for n in range(self.N+1):
                g_kRV1 = pt.exp(-self.f(X_TPFRV1 , y_observed_KBPFLRV[n], n * self.delta_t))
                #first, define g^phi
                if(n == self.N):
                    gphi_kRV1 = g_kRV1 / pt.exp(self.Z_n_(X_TPFRV1, n)).squeeze()

                elif(n == 0):
                    #calculate tildephi
                    tildeN_TPFRV1 = 100
                    ZRV1 = self.Z_n_(X_TPFRV1, n)
                    tildephi_TPFRV1 = pt.zeros_like(ZRV1).to(self.device)     
                    for TPF_i in range(tildeN_TPFRV1):

                        Ui_TPFRV1 = self.sq_delta_t * pt.randn_like(X_TPFRV1).to(self.device) + X_TPFRV1 + self.delta_t * self.b(X_TPFRV1)

                        phii_TPFRV1 = pt.exp(self.Z_n_(Ui_TPFRV1,n+1))
                        tildephi_TPFRV1 += phii_TPFRV1

                    tildephi_TPFRV1 /= tildeN_TPFRV1


                    gphi_kRV1 = g_kRV1 * tildephi_TPFRV1.squeeze()
                    ...
                    gphi_kRV1 = g_kRV1 * tildephi_TPFRV1.squeeze() / pt.exp(self.Z_n_(X_TPFRV1, n)).squeeze()
                ...
                #resample
                weightsRV1 = gphi_kRV1 / pt.sum(gphi_kRV1)
                indicesRV1 = pt.multinomial(weightsRV1, K_BPF_smallRV, replacement=True)
                ESSRV1 = 1.0 / np.sum((weightsRV1.detach().to('cpu').numpy())**2)
                ...
                for Xi in range(K_BPF_smallRV):
                    ifrejectXi = 1

                    while(ifrejectXi == 1):
                        Xi_proposeRV1 = pt.zeros_like(X_TPFRV1[0].unsqueeze(0)).to(self.device)
                        Xi_proposeRV1 = X_TPFRV1[Xi].unsqueeze(0) + self.b(X_TPFRV1[Xi].unsqueeze(0)) * self.delta_t + self.sq_delta_t * pt.randn_like(X_TPFRV1[Xi].unsqueeze(0)).to(self.device)

                        Xi_acceptrateRV1 = pt.exp(self.Z_n_(Xi_proposeRV1,n+1)).squeeze().item()

                        uniform_01 = np.random.uniform(0,1)
                        if(uniform_01 < Xi_acceptrateRV1):
                            X_TPFRV1[Xi] = Xi_proposeRV1.squeeze()#move forward
                            ifrejectXi = 0
            ...
            for n in range(self.N+1):
                g_kRV2 = pt.exp(-self.f(X_TPFRV2 , y_observed_KBPFLRV[n], n * self.delta_t))
                #first, define g^phi
                if(n == self.N):
                    gphi_kRV2 = g_kRV2 / pt.exp(self.Z_n_(X_TPFRV2, n)).squeeze()

                elif(n == 0):
                    #calculate tildephi
                    tildeN_TPFRV2 = 100
                    ZRV2 = self.Z_n_(X_TPFRV2, n)
                    tildephi_TPFRV2 = pt.zeros_like(ZRV2).to(self.device)     
                    for TPF_i in range(tildeN_TPFRV2):

                        Ui_TPFRV2 = self.sq_delta_t * pt.randn_like(X_TPFRV2).to(self.device) + X_TPFRV2 + self.delta_t * self.b(X_TPFRV2)

                        phii_TPFRV2 = pt.exp(self.Z_n_(Ui_TPFRV2,n+1))
                        tildephi_TPFRV2 += phii_TPFRV2

                    tildephi_TPFRV2 /= tildeN_TPFRV2


                    gphi_kRV2 = g_kRV2 * tildephi_TPFRV2.squeeze()
                    ...
                    gphi_kRV2 = g_kRV2 * tildephi_TPFRV2.squeeze() / pt.exp(self.Z_n_(X_TPFRV2, n)).squeeze()
                ...
                #resample
                weightsRV2 = gphi_kRV2 / pt.sum(gphi_kRV2)
                indicesRV2 = pt.multinomial(weightsRV2, K_BPF_smallRV, replacement=True)
                ESSRV2 = 1.0 / np.sum((weightsRV2.detach().to('cpu').numpy())**2)
                ...
                for Xi in range(K_BPF_smallRV):
                    ...
                    while(ifrejectXi == 1):
                        Xi_proposeRV2 = pt.zeros_like(X_TPFRV2[0].unsqueeze(0)).to(self.device)
                        Xi_proposeRV2 = X_TPFRV2[Xi].unsqueeze(0) + self.b(X_TPFRV2[Xi].unsqueeze(0)) * self.delta_t + self.sq_delta_t * pt.randn_like(X_TPFRV2[Xi].unsqueeze(0)).to(self.device)

                        Xi_acceptrateRV2 = pt.exp(self.Z_n_(Xi_proposeRV2,n+1)).squeeze().item()

                        uniform_01 = np.random.uniform(0,1)
                        if(uniform_01 < Xi_acceptrateRV2):
                            X_TPFRV2[Xi] = Xi_proposeRV2.squeeze()#move forward
                            ifrejectXi = 0

            EX2 = g_prodRV2

            print("EX2=", EX2, end='')
            print("relative variance=", math.sqrt((EX2 / EX**2) - 1))
