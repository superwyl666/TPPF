...
class mylorenzSolver():
    ...
    def b(self, x):
        ...
        drift = -xi_minus_1 * xi_minus_2 + xi_minus_1 * xi_plus_1 - x + self.alpha
    ...
    def f(self, xx, yy, t):
        xx_trimmed = xx[:, :self.d-2]
        yy_trimmed = yy[:, :self.d-2]
        y_last_two = yy[:, -2:]
        result = pt.sum((xx_trimmed - yy_trimmed).pow(2), dim=1) + pt.sum(y_last_two.pow(2), dim=1)
        result = result / (2 * self.OBsigma2)
        return result
    ...
    def train(self):
        ...
        for ii in range(ifRV):
            ...
            for n in range(self.N + 1):
                y_observed_KBPFLRV1.append(self.y_observed[n].repeat(K_BPF_smallRV,1).to(self.device))
            ...
            for n in range(self.N+1):
                ...
                if(n == self.N):
                    ...
                    for TPF_iRV1 in range(tildeN_TPFRV1):
                ...
                for XiRV1 in range(K_BPF_smallRV):
                    ifrejectXiRV1 = 1
                    
                    while(ifrejectXiRV1 == 1):
                        Xi_proposeRV1 = pt.zeros_like(X_TPFRV1[0].unsqueeze(0)).to(self.device)
                        Xi_proposeRV1 = X_TPFRV1[XiRV1].unsqueeze(0) + self.b(X_TPFRV1[XiRV1].unsqueeze(0)) * self.delta_t + self.sq_delta_t * pt.randn_like(X_TPFRV1[XiRV1].unsqueeze(0)).to(self.device)

                        Xi_acceptrateRV1 = pt.exp(self.Z_n_(Xi_proposeRV1,n+1)).squeeze().item()
                        # if(Xi == 0):
                        #     print(trynumber,'tiral,accept rate',Xi_acceptrate)
                        uniform_01RV1 = np.random.uniform(0,1)
                        if(uniform_01RV1 < Xi_acceptrateRV1):
                            X_TPFRV1[XiRV1] = Xi_proposeRV1.squeeze()#move forward
                            ifrejectXiRV1 = 0
        ...
        for ii in range(1):
            ...
            for n in range(self.N + 1):
                y_observed_KBPFLRV2.append(self.y_observed[n].repeat(K_BPF_smallRV,1).to(self.device))
            ...
            for n in range(self.N+1):
                ...
                if(n == self.N):
                    ...
                    for TPF_iRV2 in range(tildeN_TPFRV2):
                ...
                for XiRV2 in range(K_BPF_smallRV):
                    ifrejectXiRV2 = 1
                    
                    while(ifrejectXiRV2 == 1):
                        Xi_proposeRV2 = pt.zeros_like(X_TPFRV2[0].unsqueeze(0)).to(self.device)
                        Xi_proposeRV2 = X_TPFRV2[XiRV2].unsqueeze(0) + self.b(X_TPFRV2[XiRV2].unsqueeze(0)) * self.delta_t + self.sq_delta_t * pt.randn_like(X_TPFRV2[XiRV2].unsqueeze(0)).to(self.device)

                        Xi_acceptrateRV2 = pt.exp(self.Z_n_(Xi_proposeRV2,n+1)).squeeze().item()
                        # if(Xi == 0):
                        #     print(trynumber,'tiral,accept rate',Xi_acceptrate)
                        uniform_01RV2 = np.random.uniform(0,1)
                        if(uniform_01RV2 < Xi_acceptrateRV2):
                            X_TPFRV2[XiRV2] = Xi_proposeRV2.squeeze()#move forward
                            ifrejectXiRV2 = 0
