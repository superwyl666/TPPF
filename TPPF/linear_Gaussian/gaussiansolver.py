...
from function_space import DenseNet, Linear, NN, NN_Nik, SingleParam, MySequential, DenseNet_tanh, DenseNet1, DenseNet2, DenseNet3
from utilities import compute_test_error, do_importance_sampling, do_importance_sampling_me


class mymySolver_problemdependent():
    ...
    def __init__(self, name, problem, lr=0.01, L=500, K=500, delta_t=0.25,
                 ...
                 K_BPF_large = 10000000, K_BPF_small = 200, phi0_BPF = 1, replicate_num = 1000, Z_true = 1, resample_kappa = 1):
    ...
    def train(self):
        ...
        # print("relative cariance done")
        # print('running time for Var:',time_2 - time_1,end = ' ')
        # Rel_Var2 = (Z_sum_forRV / K_forRV - phi0 * phi0) / (phi0 * phi0)
        # print('after',  'training, Rel_Var^2 = ', Rel_Var2)
        ...
        #for ii in range(0):
            ...
            for n in range(self.N+1):
                ...
                # if not pt.isfinite(gphi_k).all():
                #     raise RuntimeError("NaN/Inf in gphi_k")

                weights = gphi_k / pt.sum(gphi_k)
                weights = pt.clamp(weights, min=0.0)
                weights = weights / pt.sum(weights)

                ESS = 1.0 / pt.sum(weights ** 2)
                ESS = ESS.item()
...
#                     ithESSnum += 1
        ...
        ifRV = 1
        #ifRV = 0 #set ifRV = 0 if not computing relative variance
        

        pt.cuda.empty_cache()

        print("begin calculating the relative variance")
        K_forRV = 100000
        if(self.d >= 10):
            K_forRV = 50000

        for ii in range(ifRV):

            X_TPF_RV = self.X_0.repeat(K_forRV, 1).to(self.device)
                
            y_observed_KBPFL_RV = []#define y_OB for K_BPF_large particles
            for n in range(self.N + 1):
                y_observed_KBPFL_RV.append(self.y_observed[n].repeat(K_forRV,1).to(self.device))

            g_prod_RV = 1

            for n in range(self.N+1):

                g_k_RV = pt.exp(-self.f(X_TPF_RV - y_observed_KBPFL_RV[n], n * self.delta_t))
            
                if(n == self.N):
                    gphi_k_RV = g_k_RV / self.phi_Z_n_(X_TPF_RV,n).squeeze()
                                     
                elif(n == 0):
                    gphi_k_RV = g_k_RV * self.tildephi_Z_n_(X_TPF_RV, n).squeeze()

                else:
                    gphi_k_RV = g_k_RV * self.tildephi_Z_n_(X_TPF_RV,n).squeeze() / self.phi_Z_n_(X_TPF_RV,n).squeeze()

                g_prod_RV *= (gphi_k_RV**2).mean().item()#EX2
                    
                    
                    
                    
                    
                # --- compute weights_RV safely ---
                weights_RV = gphi_k_RV / pt.sum(gphi_k_RV)
                weights_RV = weights_RV + 1e-12

                # if not pt.isfinite(weights_RV).all():
                #     raise RuntimeError("NaN/Inf in weights_RV")

                weights_RV = pt.clamp(weights_RV, min=0.0)
                weights_RV = weights_RV / pt.sum(weights_RV)

                # --- ESS ---
                ESS_RV = 1.0 / pt.sum(weights_RV ** 2)
                ESS_RV = ESS_RV.item()

                # --- resample ONLY if needed ---
                if ESS_RV <= self.resample_kappa * K_forRV:
                    indices_RV = pt.multinomial(weights_RV, K_forRV, replacement=True)
                    X_TPF_RV = X_TPF_RV[indices_RV]
#                 #resample
#                 weights_RV = gphi_k_RV / pt.sum(gphi_k_RV)
#                 indices_RV = pt.multinomial(weights_RV, K_forRV, replacement=True)
#                 ESS_RV = 1.0 / pt.sum(weights_RV**2)
...
#                     X_TPF_RV = X_TPF_RV[indices_RV]
                ...
                X_TPF_RV = (self.delta_t * self.mean_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)).expand(X_TPF_RV.shape[0],-1)\
                                + self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)) * (X_TPF_RV + self.delta_t * self.b(X_TPF_RV)))\
                        / (self.delta_t + self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)))\
                            + pt.sqrt((self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)) * self.delta_t)\
                                        / (self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)) + self.delta_t))\
                                * pt.randn_like(X_TPF_RV).to(self.device)
                
                #X_TPF_RV += self.delta_t * self.b(X_TPF_RV) + self.sq_delta_t * pt.randn_like(X_TPF_RV).to(self.device)
        ...
        for ii in range(1):
            ...
            X_TPF_RV11 = self.X_0.repeat(K_forRV, 1).to(self.device)
                
            y_observed_KBPFL_RV11 = []#define y_OB for K_BPF_large particles
            for n in range(self.N + 1):
                y_observed_KBPFL_RV11.append(self.y_observed[n].repeat(K_forRV,1).to(self.device))

            g_prod_RV11 = 1

            for n in range(self.N+1):

                g_k_RV11 = pt.exp(-self.f(X_TPF_RV11 - y_observed_KBPFL_RV11[n], n * self.delta_t))
            
                if(n == self.N):
                    gphi_k_RV11 = g_k_RV11 / self.phi_Z_n_(X_TPF_RV11,n).squeeze()
                                     
                elif(n == 0):
                    gphi_k_RV11 = g_k_RV11 * self.tildephi_Z_n_(X_TPF_RV11, n).squeeze()

                else:
                    gphi_k_RV11 = g_k_RV11 * self.tildephi_Z_n_(X_TPF_RV11,n).squeeze() / self.phi_Z_n_(X_TPF_RV11,n).squeeze()

                g_prod_RV11 *= (gphi_k_RV11).mean().item()#EX

               
                # --- compute weights_RV safely ---
                weights_RV11 = g_k_RV11 / pt.sum(g_k_RV11)
                weights_RV11 = weights_RV11 + 1e-12

                # if not pt.isfinite(weights_RV11).all():
                #     raise RuntimeError("NaN/Inf in weights_RV11")

                weights_RV11 = pt.clamp(weights_RV11, min=0.0)
                weights_RV11 = weights_RV11 / pt.sum(weights_RV11)

                # --- ESS ---
                ESS_RV11 = 1.0 / pt.sum(weights_RV11 ** 2)
                ESS_RV11 = ESS_RV11.item()

                # --- resample ONLY if needed ---
                if ESS_RV11 <= self.resample_kappa * K_forRV:
                    indices_RV11 = pt.multinomial(weights_RV11, K_forRV, replacement=True)
                    X_TPF_RV11 = X_TPF_RV11[indices_RV11]
                    
                    
#                 #resample
#                 weights_RV11 = g_k_RV11 / pt.sum(g_k_RV11)
#                 indices_RV11 = pt.multinomial(weights_RV11, K_forRV, replacement=True)
#                 ESS_RV11 = 1.0 / np.sum((weights_RV11.detach().to('cpu').numpy())**2)

#                 if(ESS_RV11 <= self.resample_kappa * K_forRV):
#                     X_TPF_RV11 = X_TPF_RV11[indices_RV11]
                    
                X_TPF_RV11 = (self.delta_t * self.mean_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)).expand(X_TPF_RV11.shape[0],-1)\
                                + self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)) * (X_TPF_RV11 + self.delta_t * self.b(X_TPF_RV11)))\
                        / (self.delta_t + self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)))\
                            + pt.sqrt((self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)) * self.delta_t)\
                                        / (self.var_z_n(pt.tensor([[n+1]],dtype=pt.float32).to(self.device)) + self.delta_t))\
                                * pt.randn_like(X_TPF_RV11).to(self.device)
                
                #X_TPF_RV11 += self.delta_t * self.b(X_TPF_RV11) + self.sq_delta_t * pt.randn_like(X_TPF_RV11).to(self.device)
        ...
        print("RV done", "EX2=", EX2, "EX =", EX, "RV=", math.sqrt((EX2 / (EX)**2) - 1))
...
class mySolver():
    ...
    def __init__(self, name, problem, lr=0.01, L=500, K=500, delta_t=0.25,
                 ...
                 seed=42, save_results=False, u_l2_error_flag=True, log_gradient=False,  verbose=False, T = 0.5,
                 sampling_method = 'untwisted', train_goal = 'RE', d = 2, OBsigma2 = 1, y_observed = [],
                 K_BPF_large = 10000, K_BPF_small = 200, phi0_BPF = 1, replicate_num = 1000, Z_true = 1):
        self.problem = problem
        self.name = name
        self.date = date.today().strftime('%Y-%m-%d')
        self.d = d
        ...
        if self.approx_method == 'control':
            ...
            if self.time_approx == 'outer':
                ...
                self.z_n = DenseNet(d_in=self.d + 1, d_out=1, lr=self.lr, seed=123)
                #self.z_n = MySequential(d_in=self.d + 1, d_out=self.d, lr=self.lr, seed=123)
                #self.z_n = MySequential(d_in=self.d + 1, d_out=1, lr=self.lr, seed=123)
    ...
    def train(self):
        ...
        for l in range(self.L):
            ...
            if(l % 1 == 0 and (1 == 0)):
                ...
                K_forRV = 2000
                K_forRV_inner = 50
                Zsum_forRV1 = pt.zeros(K_forRV).to(self.device)
                Zsum_forRV2 = pt.zeros(K_forRV).to(self.device)

                X_forRV = self.X_0.repeat(K_forRV, 1).to(self.device)

                y_observed_forRV = []
                for n in range(self.N + 1):
                    y_observed_forRV.append(self.y_observed[n].repeat(K_forRV,1).to(self.device))

                Zsum_forRV1 += -2 * self.f(X_forRV-y_observed_forRV[0],0)

                Z = self.Z_n_(X_forRV, 0)

                for n in range(self.N):
                    #first calculate the normalizing constant
                    normalized = pt.zeros_like(Z).to(self.device)
                    for n_normalized in range(K_forRV_inner):
                        Ui_inner = X_forRV + self.delta_t * self.b(X_forRV) + self.sq_delta_t * pt.randn_like(X_forRV).to(self.device)
                        normalized += pt.exp(self.Z_n_(Ui_inner,n+1))
                    normalized /= K_forRV_inner

                    #move forward
                    X_forRV = X_forRV + self.delta_t * self.b(X_forRV) + self.sq_delta_t * pt.randn_like(X_forRV).to(self.device)
                    
                    #calculate summations
                    Zsum_forRV1 += -2 * self.f(X_forRV-y_observed_forRV[n+1], (n+1) * self.delta_t)
                    Zsum_forRV2 += - (self.Z_n_(X_forRV, (n+1)).squeeze() - pt.log(normalized).squeeze())

                ttt2 = time.time()
                #calculate relative variance
                Rel_Var2 = ((pt.exp(Zsum_forRV1) * pt.exp(Zsum_forRV2)).mean().item() - phi0 * phi0) / (phi0 * phi0)
                if(l % self.print_every == 0):
                    print('after', l, 'training, Rel_Var^2 = ', Rel_Var2,'running time for Var:', ttt2-ttt1, end= ' ')
                
                # '''
                # calculate relative variance
                # avoid use large vector here to save memory
                # '''
                # K_forRV = 500
                # K_forRV_inner = 50
                ...
                # Z_sum_forRV = 0
                # time_1 = time.time()
                # for MCsample in range(K_forRV):
                    
                #     Zsum_forRV1, Zsum_forRV2 = 0, 0

                #     X_forRV = self.X_0.repeat(1, 1).to(self.device)
                    
                    

                #     Zsum_forRV1 += -2 * self.f(X_forRV,0).item()
                ...
                #         #calculate summations
                #         Zsum_forRV1 += -2 * self.f(X_forRV, (n+1) * self.delta_t).item()
                #         Zsum_forRV2 += - (self.Z_n_(X_forRV, (n+1) * self.delta_t) - pt.log(normalized)).item()

                #     Z_sum_forRV += np.exp(Zsum_forRV1) * np.exp(Zsum_forRV2)
                    
                # #calculate relative variance
                # time_2 = time.time()
                # print('running time for Var:',time_2 - time_1,end = ' ')
                # Rel_Var2 = (Z_sum_forRV / K_forRV - phi0 * phi0) / (phi0 * phi0)
                # print('after', l, 'training, Rel_Var^2 = ', Rel_Var2)
            ...
            for n in range(self.N):
                ...
                if (self.sampling_method == 'reparameterize'):
                    ...
                    B: 1-d tensor
                    C: 2-d tensor
                    X = B/A + sqrt(C/A - (B/A)otimes2) cdot N(0,1)
                    '''

                #calculate A,B,C using the same samples from untwisted markov kernel
                    tildeN = 200#inner MC size

                #calculate A,B,C simotanuously
                # print('X',X.size())
                # print('Z_n_',self.Z_n_(X,n).size())
                    repa_sumA = pt.zeros_like(Z).to(self.device)
                    repa_sumB = pt.zeros_like(X).to(self.device)
                    repa_sumC = pt.zeros_like(pt.einsum('ij,ik->ijk', X, X)).to(self.device)
                
                    for repa_i in range(tildeN):
                        repara_Ui = self.sq_delta_t * pt.randn_like(X).to(self.device) + X + self.delta_t * self.b(X)
                        repa_phii = pt.exp(self.Z_n_(repara_Ui,n+1))

                        repa_sumA += repa_phii
                        repa_sumB += repara_Ui * repa_phii
                        repa_sumC += pt.einsum('ij,ik->ijk', repara_Ui, repara_Ui) * repa_phii.unsqueeze(-1)
 

                    repa_sumA = repa_sumA / tildeN
                    repa_sumB = repa_sumB / tildeN
                    repa_sumC = repa_sumC / tildeN
                # print('ABC done')
                # print('A',repa_sumA.size())
                # print('B',repa_sumB.size())
                # print('C',repa_sumC.size())

                #update X
                    repa_mean = repa_sumB / repa_sumA
                # if n==5:
                #     print('mean of delta X', repa_mean[1])
                    repa_cov = repa_sumC / repa_sumA.unsqueeze(-1) - pt.einsum('ij,ik->ijk', repa_mean, repa_mean)
                # if n==5:
                #     print("cov of delta X",repa_cov[1])
                #print('mean,cov done')
                    try:
                        Xnew = repa_mean + pt.distributions.MultivariateNormal(pt.zeros_like(repa_mean).to(self.device), repa_cov).sample()
                ...
                #print("reparamerize done!")
                

                if(self.sampling_method == 'rejectsampling'):
                    '''
                    reject sampling 
                    move forward with original MC chain, accept with prob. phi i.e. exp(NN(Xproposal,n+1)) until first accepted
                    '''
                    #first calculate the normalizing constant with the old X,X_untwisted, denote by repa_sumA,repa_sumA_untwisted
                    tildeN = 500
                    ...
                    for repa_i in range(tildeN):
                        ...
                        repa_phii_untwisted = pt.exp(self.Z_n_(repara_Ui_untwisted,n+1))
                        repa_sumA_untwisted += repa_phii_untwisted
                    repa_sumA /= tildeN#tildephi for twisted model
                    repa_sumA_untwisted /= tildeN#tilde phi for untwisted model
                    ...
                    for Xi in range(self.K):
                        ...
                        trynumber = 0
                        while(ifrejectXi == 1):
                            Xi_propose = pt.zeros_like(X[0].unsqueeze(0)).to(self.device)
                            Xi_propose = X[Xi].unsqueeze(0) + self.b(X[Xi].unsqueeze(0)) * self.delta_t + pt.randn_like(X[Xi].unsqueeze(0)).to(self.device)
                            ...
                            if(uniform_01 < Xi_acceptrate):
                                X[Xi] = Xi_propose.squeeze()
                                ifrejectXi = 0
                            else:
                                trynumber += 1
                                averagetry += 1
                    # print("reject sampling done! average reject:", averagetry / self.K)
