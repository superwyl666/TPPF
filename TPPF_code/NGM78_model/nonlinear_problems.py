...
class LLGC_general_f():
    ...
    def __init__(self, name='LLGC', d=1, off_diag=0, T=5, seed=42):
        ...
        self.d = d
        self.T = T
        self.A = (-pt.eye(self.d) + 0 * off_diag * pt.randn(self.d, self.d)).to(device)
        self.B = (pt.eye(self.d) + off_diag * pt.randn(self.d, self.d)).to(device)
        self.alpha = -pt.ones(self.d, 1).to(device)
        self.X_0 = pt.zeros(self.d).to(device)
...
class LQGC():
    ...
    '''
    def __init__(self, name='LQGC', delta_t=0.05, d=2, off_diag=0, T=5, seed=42):
        ...
        self.X_0 = pt.zeros(self.d)+0.5

        if ~np.all(np.linalg.eigvals(self.A.cpu().numpy()) < 0):
            print('not all EV of A are negative')

        self.P = 1 * 0.5 * pt.eye(self.d).to(device)
    ...
    def sigma(self, x):
        return self.B
    # '''
    # let us first try yt = X_t + B_{t+t_0} with dX = AXdt + dW, 
    # in this case f = -|x-yt|^2/2
    # g = -|x-yT|^2/2
    # we run SDE for one time to get yt
    # '''
    # def f(self,x,t):#my path integral
    #     diff = x - self.yt[int(t / self.delta_t), :]
    #     weighted_diff = pt.mm(self.P/(t+self.generate_yt_t0), diff.unsqueeze(1)).squeeze()
    #     fxt = pt.sum(weighted_diff * diff)
    #     return fxt
    
    # def g(self,x):#my terminal
    #     diff = x - self.yt[int(self.T / self.delta_t), :]
    #     weighted_diff = pt.mm(self.P/(self.T+self.generate_yt_t0), diff.unsqueeze(1)).squeeze()
    #     fxt = pt.sum(weighted_diff * diff)
    #     return fxt
