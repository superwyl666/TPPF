...
class DenseNet(pt.nn.Module):
    def __init__(self, d_in, d_out, lr, arch=[10, 10], seed=42):
    ...
    def forward(self, x):
        ...
        x = 2 * (pt.nn.Tanh()(x) - 1)  # Scale Tanh output from [-1, 1] to [-4, 0]
        return x


'''
this dense net are specifically defigned for problem-dependent twisting, phi = c_k N(cdot;mean,var)
'''
class DenseNet1(pt.nn.Module):#coefficient ck
...
class DenseNet2(pt.nn.Module):#mean
    def __init__(self, d_in, d_out, lr, arch=[10, 10], seed=42):
...
class DenseNet3(pt.nn.Module):#var
    def __init__(self, d_in, d_out, lr, arch=[10, 10], seed=42):
        ...
        for i in range(len(self.nn_dims) - 1):
            if i == len(self.nn_dims) - 2:
                ...
                x = pt.cat([x, pt.nn.functional.relu(pt.matmul(x, self.W[2 * i])
                                                     + self.W[2 * i + 1]) ** 2], dim=1)
        #x = 2 * (pt.nn.Tanh()(x) - 1)  # Scale Tanh output from [-1, 1] to [-4, 0]
        x = pt.nn.Softplus()(x)#make the variance positive
        #x = pt.exp(x)
        #x = pt.nn.functional.relu(x)+0.0001
        return x


# class DenseNet3(pt.nn.Module):
#     def __init__(self, d_in, d_out, lr, arch=[30, 30], seed=42):
#         super(DenseNet3, self).__init__()
#         pt.manual_seed(seed)
#         self.nn_dims = [d_in] + arch + [d_out]
#         self.W = [item for sublist in
#                   [[pt.nn.Parameter(pt.randn(self.nn_dims[i], self.nn_dims[i + 1]) * 0.1),
#                     pt.nn.Parameter(pt.zeros(self.nn_dims[i + 1]))] for
#                    i in range(len(self.nn_dims) - 1)]
#                   for item in sublist]

#         for i, w in enumerate(self.W):
#             self.register_parameter('param_%d' % i, w)
...
#                 x = pt.matmul(x, self.W[2 * i]) + self.W[2 * i + 1]
#             else:
#                 # 确保 x 的形状正确，在 dim=1 进行拼接
#                 x = pt.cat([x, pt.nn.functional.relu(pt.matmul(x, self.W[2 * i])
#                                                      + self.W[2 * i + 1]) ** 2], dim=1)
#                 print(f"Layer {i}, x shape: {x.shape}")
#         x = x * x  # make the variance positive
#         return x
...
class MySequential(pt.nn.Module):
    ...
    def forward(self, x):
        ...
        # Apply custom scaling to bound output between -10 and 0
        x = 2 * (pt.nn.Tanh()(x) - 1)  # Scale Tanh output from [-1, 1] to [-4, 0]
        return x
